...
<body>
    <div class="header">
        <h1>LAPORAN DELIVERY CONTAINER</h1>
    ...
    <div class="summary">
        <div class="summary-item">
            <h3>{{ $deliveries->where('status', 'pending')->count() }}</h3>
            <p>Pending</p>
        </div>
        <div class="summary-item">
            <h3>{{ $deliveries->where('status', 'confirmed')->count() }}</h3>
            <p>Confirmed</p>
        </div>
        <div class="summary-item">
            <h3>{{ $deliveries->where('status', 'rejected')->count() }}</h3>
            <p>Rejected</p>
        </div>
        <div class="summary-item">
        ...
        <tbody>
            @forelse($deliveries as $index => $delivery)
                <tr>
                    ...
                    <td>{{ $delivery->luggage }}</td>
                    <td>
                        <span class="liquid-status liquid-{{ $delivery->liquid_status }}">
                    ...
                    <td>
                        {{ $delivery->block ? 'Block ' . $delivery->block->name : 'Not Assigned' }}
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="9" style="text-align: center; color: #666;">Tidak ada data delivery</td>
                </tr>
            @endforelse
        </tbody>
    </table>
