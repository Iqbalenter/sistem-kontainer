<div class="bg-white rounded-lg shadow-md p-6 w-96">
    ...
    <div class="text-xs text-gray-500 mb-4">
        Delivery ID: {{ session('delivery_data.id') }}
    </div>
    ...
    <div class="bg-gray-100 p-4 rounded-lg space-y-2">
        ...
        <div class="flex justify-between">
            ...
            <span class="text-gray-800">{{ $luggage }}</span>
        </div>
        <div class="flex justify-between">
            <strong class="text-gray-700">Tipe Muatan:</strong>
            <span class="text-gray-800">{{ $liquid_status }}</span>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    let deliveryId = '{{ session('delivery_data.id') }}';
    console.log('Delivery ID:', deliveryId); // Debug log
    ...
    window.checkStatus = async function() {
        ...
        try {
            console.log('Checking status for delivery:', deliveryId); // Debug log
            const response = await fetch(`/delivery/${deliveryId}/check-status`, {
                method: 'GET',
                headers: {
